...
public class ConstantPropagation extends
    ...
    public CPFact newBoundaryFact(CFG<Stmt> cfg) {
        ...
        cfg.getIR().getParams().forEach(var -> {
            if (canHoldInt(var)) {
                fact.update(var, Value.getNAC());//All initialized to NAC
            }
        });
        return fact;
    }
    ...
    public void meetInto(CPFact fact, CPFact target) {
        ...
        fact.forEach(((var, value) -> {
            Value oldValue = target.get(var);
            target.update(var, meetValue(value, oldValue));
        }));
    }
    ...
    public boolean transferNode(Stmt stmt, CPFact in, CPFact out) {
        ...
        if (stmt instanceof DefinitionStmt<?, ?> s) {
            if (s.getLValue() instanceof Var var && canHoldInt(var)) {
                ...
                return !oldVal.equals(newVal) || changed.get();
            }
        }
        return changed.get();
    }
    ...
    public static Value evaluate(Exp exp, CPFact in) {
        ...
        if (exp instanceof IntLiteral e) {
            ...
            if (v2.isConstant() && v2.getConstant() == 0 && b.getOperator() instanceof ArithmeticExp.Op op) {
                if (op == ArithmeticExp.Op.REM || op == ArithmeticExp.Op.DIV) {
                    return Value.getUndef();
                }
            }
            if (v1.isConstant() && v2.isConstant()) { // if v1 and v2 are constants
            ...
            } else if (v1.isNAC() || v2.isNAC()) {// if v1 or v2 is NAC
                return Value.getNAC();
            }
            return Value.getUndef();
        }
